...
public class DrivingRecordMapPresent extends BasePresenter<DrivingRecordMapModel, PositionView> {
    ...
    public void getPositionDataByDate(String date) {
        ...
        mModel.getPositionDataByDate(date).subscribe(new Subscriber<ResponseDataBean<List<PositionInfo>>>() {
            @Override
            ...
            @Override
            public void onNext(ResponseDataBean<List<PositionInfo>> positionData) {
                if(positionData.getData()!=null){
                    List<LatLng> latLngs=new ArrayList<>();
                    for (PositionInfo position:positionData.getData()) {
