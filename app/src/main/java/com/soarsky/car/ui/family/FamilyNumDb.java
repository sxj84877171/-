...
public class FamilyNumDb {
    ...
    public boolean isNuSyncTerminal(){
        ...
        if(null!=App.getApp().getCommonParam().getCarNum()&&getNuTerminal().size()>0){
            return  true;
        }
        return false;
    }
    ...
    public List<FamilyNum> getNuTerminal(){
        ...
        if(null!=App.getApp().getCommonParam().getCarNum()){
            return daoSession.getFamilyNumDao().queryBuilder().where(FamilyNumDao.Properties.Tstate.eq(0), FamilyNumDao.Properties.Carnum.eq(App.getApp().getCommonParam().getCarNum())).distinct().list();
        }
