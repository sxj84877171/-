package com.soarsky.car.ui.roadside.list;

import com.soarsky.car.base.BasePresenter;
import com.soarsky.car.ui.roadside.RoadSideModel;
import com.soarsky.car.ui.roadside.RoadSideView;

import rx.Subscriber;

/**
 * Andriod_Car_App
 * 作者： 苏云
 * 时间： 2016/12/19
 * 公司：长沙硕铠电子科技有限公司
 * Email：suyun@soarsky-e.com
 * 公司网址：http://www.soarsky-e.com
 * 公司地址（Add）  ：湖南省长沙市岳麓区麓谷信息港A座8楼
 * 版本：1.0.0.0
 * 邮编：410000
 * 程序功能：
 * 该类为
 */


public class RoadSideListPresent extends BasePresenter<RoadSideListModel,RoadSideListView>{

    @Override
    public void onStart() {

    }

    /**
     * 获取故障救援列表
     */
    public void getRescueList(String carnum){
        mView.showProgess();
        mModel.getRescueList(carnum).subscribe(new Subscriber<RoadSideListParam>() {
            @Override
            public void onCompleted() {

                mView.stopProgess();

            }

            @Override
            public void onError(Throwable e) {
                mView.stopProgess();
                mView.getRescueListFail();

            }

            @Override
            public void onNext(RoadSideListParam roadSideListParam) {
                mView.stopProgess();
                mView.getRescueListSuccess(roadSideListParam);
            }
        });
    }


}
