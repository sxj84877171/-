...
public class RoadSideListOrderActivity extends BaseActivity<RoadSideListOrderPresent,RoadSideListOrderModel> implements RoadSideListOrderView,View.OnClickListener{
    ...
    /**
     * 取消故障救援记录成功
     * @param roadSideListOrderDelRescueParam
     */
    @Override
    public void delRescueByIdSuccess(RoadSideListOrderDelRescueParam roadSideListOrderDelRescueParam) {
        ...
        if (roadSideListOrderDelRescueParam != null){
            dialog.dismiss();
            if("0".equals(roadSideListOrderDelRescueParam.getStatus())){

                finish();
            }else {
                ToastUtil.show(this,roadSideListOrderDelRescueParam.getMessage());
            }
        }
